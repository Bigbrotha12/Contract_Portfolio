...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    error NFTDemo__Unauthorized();
    error NFTDemo__Initializing();
    error NFTDemo__Initialized();
    error NFTDemo__MissingInterface();
    error NFTDemo__FailedInitialization(bytes errorData);
    error NFTDemo__ZeroAddress();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function getVersion() external ifAdmin returns (string memory version) {
        address target_ = s_callRouting_[address(0)];
        version = s_version_[target_];
    }

    /// @notice Returns route for given role via event
    function getRouting(address _role) external ifAdmin returns(address route_) {
        route_ = s_callRouting_[_role];
    }

    function _implementation() internal view override returns (address route) {
        if(msg.sender == s_admin_) revert NFTDemo__Unauthorized();
    ...
    function upgradeInit(IERC165 _impl, bytes[] calldata _initData) external ifAdmin {
        if(s_initializing_) revert NFTDemo__Initializing();
        if(s_initialized_[address(_impl)]) revert NFTDemo__Initialized();
        ...
        if(!validTarget) revert NFTDemo__MissingInterface();
        ...
        if(!success) revert NFTDemo__FailedInitialization(err);
    ...
    function changeAdmin(address _newAdmin) external ifAdmin {
        if(_newAdmin == address(0)) revert NFTDemo__ZeroAddress();
    ...
    function changeRouting(address _role, address _target) external ifAdmin {
        if(_role == address(0)) revert NFTDemo__ZeroAddress();
