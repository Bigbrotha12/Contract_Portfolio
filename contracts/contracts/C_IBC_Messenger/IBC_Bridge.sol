...
contract IBC_Bridge is EIP712X {
    ...
    constructor(string memory _name, string memory _version, address _minter) EIP712(_name, _version) {
    ...
    function dataSend(
      ...
      uint256 _receivingChainId
        ...
        require(Demo.balanceOf(msg.sender) >= _amount, "IBC_Bridge: Insufficient funds");

        uint256 nonce_ = nonce[msg.sender][block.chainid][_receivingChainId]++;

        Demo.burnFrom(msg.sender, _amount);
        emit DataSent(_receiver, _amount, _receivingChainId, nonce_, destinationDomain);
        return true;
    }
    ...
    function dataReceive(
      ...
      bytes32 structHash = buildStructHash(_receiver, block.chainid, _amount, nonce_);
