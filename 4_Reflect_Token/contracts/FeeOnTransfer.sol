...
contract FeeOnTransfer is ERC20, Pausable, Ownable {
  ...
  constructor (
    ...
    for(uint i = 0; i < acquisitionWallets.length; i++){
      ...
      rOwned[acquisitionWallets[i]] +=
        rTotal / 100 * 3 / acquisitionWallets.length;
  ...
  function getAcquisitionWallet(uint256 _index) public view returns (address){
  ...
  /// @dev reflection fees are deducted from rSupply by {_reflectFee} function,
  ...
  function removeExchange(address _exchangePair) external onlyOwner {
    require(isExchange[_exchangePair], "FeeOnTransfer: Address not Exchange");
  ...
  function changeMarketing(address _newAddress) external onlyOwner {
    require(_newAddress != address(0), "FeeOnTransfer: Address cannot be zero address");
  ...
  function changeAcquisition(
    ...
    require(_index < acquisitionWallets.length, "FeeOnTransfer: Invalid index value");
    require(_newAddress != address(0), "FeeOnTransfer: Address cannot be zero address");
  ...
  function excludeAccount(address _account) external onlyOwner {
    require(!isExcluded[_account], "FeeOnTransfer: Account already excluded");
    require(balanceOf(_account) < tTotal, "FeeOnTransfer: Cannot exclude total supply");
  ...
  function includeAccount(address _account) external onlyOwner {
    require(isExcluded[_account], "FeeOnTransfer: Account already included");
