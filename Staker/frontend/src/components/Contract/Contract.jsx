import { Card, Form, notification, Input } from "antd";
...
export default function Contract() {
  ...
  const contractAddress = useMemo(() => {
    if (!isDeployedToActiveChain && !targetAddress) return null;
    if (targetAddress) return targetAddress;
    return contract.networks[parseInt(chainId, 16)]?.["address"] || null;
  }, [targetAddress, chainId, contract, isDeployedToActiveChain]);
      ...
      <Card
        title={
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "left", fontWeight: "normal" }}>
              Target Address:
              <Input style={{ marginLeft: "10px"}} placeholder="input placeholder" onChange={e => setTargetAddress(e.target.value)} />
            </div>
          </>
        }
        size="large"
