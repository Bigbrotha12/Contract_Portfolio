"use strict";
const feeOnBuySell = artifacts.require("FeeOnTransfer");
...
contract("ERC20 functionality", (accounts) => {
    ...
    it("should transfer 30 tokens to second account", async () => {
        const instance = await feeOnBuySell.deployed();
        const initBalOne = await instance.balanceOf(accounts[1]);
        const initBalZero = await instance.balanceOf(accounts[0]);
        await instance.transfer(accounts[1], 30 * 10 ** decimal);
        const endBalOne = await instance.balanceOf(accounts[1]);
    ...
    it("should allow account two to transfer 20 tokens from first account to third account", async () => {
        const instance = await feeOnBuySell.deployed();
        const initBalTwo = await instance.balanceOf(accounts[2]);
        const initBalZero = await instance.balanceOf(accounts[0]);
        await instance.approve(accounts[1], 20 * 10 ** decimal);
        await instance.transferFrom(accounts[0], accounts[2], 20 * 10 ** decimal, {
            from: accounts[1],
        });
        const endBalTwo = await instance.balanceOf(accounts[2]);
        const endBalZero = await instance.balanceOf(accounts[0]);
    ...
    it("should not allow an account to transfer from another account without approval", async () => {
        const instance = await feeOnBuySell.deployed();
        await truffleAssert.reverts(instance.transferFrom(accounts[0], accounts[2], 20 * 10 ** decimal, {
            from: accounts[1],
...
contract("Pausable features", (accounts) => {
    it("should allow admin to pause token transfers", async () => {
        ...
        await truffleAssert.reverts(instance.transfer(accounts[1], 30 * 10 ** 13), "Pausable: paused");
        await instance.approve(accounts[1], 30 * 10 ** decimal);
        await truffleAssert.reverts(instance.transferFrom(accounts[0], accounts[1], 30 * 10 ** decimal, {
            from: accounts[1],
        }), "Pausable: paused");
    });
    it("should allow admin to un-pause token transfers", async () => {
        ...
        await truffleAssert.passes(instance.transfer(accounts[1], 30 * 10 ** decimal), "transfer attempt failed");
    });
});
...
contract("Fee redistribution features", (accounts) => {
    ...
    it("should transfer 1% of transaction to marketing wallet when buying token", async () => {
        ...
        const marketingFee = ((100 * 10 ** decimal).toString() * _buyFeeMarketing) / 100;
        ...
        assert.equal(transferZero.toString(), (((100 * (100 - _buyFeeMarketing - _buyFeeAcquisition)) / 100) *
            10 ** decimal).toString(), "Transfer fee not deducted correctly");
    ...
    it("should transfer 1% of transaction to all holders wallet (reflection) when buying token", async () => {
        ...
        assert(Math.abs(reflectFee * (endBalance / supply) - transferBalance) === 0, "Reflection was not calculated correctly");
    });
    ...
    it("should reimburse re-included accounts for any missed reflections", async () => {
        ...
        assert(Math.abs(missedReflections - transferBalance) === 0, "Reflection was not reimbursed correctly");
    });
